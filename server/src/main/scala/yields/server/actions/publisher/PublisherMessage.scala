...
import yields.server.actions.exceptions.{UnauthorizeActionException, ActionArgumentException}
import yields.server.actions.nodes.NodeMessageRes
...
case class PublisherMessage(nid: NID, text: Option[String], contentType: Option[String], content: Option[Blob])
  ...
  override def run(metadata: Metadata): Result = {
    ...
    if (publisher.users.contains(metadata.client)) {
      ...
      throw new UnauthorizeActionException("only registered user can publish in a publisher")
    }
  }
